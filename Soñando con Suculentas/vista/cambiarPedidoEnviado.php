    ...
    <head>
        ...
        <script>
            function filtrar() {
                ...
                $.ajax({
                    url : "../filtros/buscadorSetEnviado.php",
                    method : "POST",
                    data : {filtro : filtro}
        ...
        <title>Establecer un pedido a enviado</title>
    </head>
<body>
    <?php
    
    if (isset($pedidos)) {
        echo '<h1 class="letraCursiva text-center display-5 mb-4 pt-3 colorVerde">Establecer un pedido como enviado</h1>';

        if (count($pedidos) == 0) {
            echo '<h2 class="letraCursiva text-center display-5 my-5 pt-3 ">No hay pedidos pendientes</h2>
                            ...
                            <th scope="col">ID de pedido</th><th scope="col">Usuario</th><th scope="col">Factura</th><th scope="col">Fecha del pedido</th><th scope="col" class="text-center">Acciones</th>
                                ...
                                foreach ($pedidos as $pedido) {
                                    echo '<tr>
                                        ...
                                        <td class="text-center">' . $pedido["EMAIL_USUARIO"] . '</td>
                                        <td class="text-center">
                                            <a href="' . $pedido["FACTURA"] .'" target="_blank" class="btn boton2 px-5">Ver factura</a> 
                                        </td>
                                        <td class="text-center">' . $pedido["FECHA"] . '</td>
                                        <td class="text-center">
                                            <form action="../controlador/ctrPedidosNoEnviados.php" method="post">
                                                <input type="submit" class="btn boton2 mb-1 mb-xl-0" value="Establecer como enviado">
